-- ----------------------------------------------------------------------------
-- 0008 — Random-mode difficulty (easy/medium/hard) + Player Pick advance on an
--        invalid (no common player / same club) combination.
--
--   * rooms.difficulty: lobby setting that only affects Random challenge picking.
--       - easy   : prefer combinations with MANY common players (>= 6).
--       - hard   : prefer combinations with FEW common players (<= 3).
--       - medium : target count decreases each round (8, 7, 6 ... down to 2).
--   * Player Pick: when the two picks have no common player (or the same club),
--     the round is ended (no winner) with an invalid_reason and the normal
--     both-ready gate moves the match to a fresh selection round.
-- ----------------------------------------------------------------------------
...
-- ---- create_round: difficulty-aware Random selection -----------------------
...
begin
  ...
  -- -------- Random mode with difficulty-driven combination selection --------
...
-- ---- Forward the room difficulty everywhere create_round is called ----------
...
-- ---- set_room_settings: accept difficulty ---------------------------------
...
-- ---- Player Pick: invalid combination ends the round (no winner) -----------
...
begin
  ...
  if v_reason is not null then
    -- No common player (or the same club was picked): end this round with no
    -- winner. The both-ready gate then moves the match to a fresh selection
    -- round. We keep the selections hidden (do not publish the challenge).
    update public.rounds
      set state = 'finished',
          finished_at = now(),
          no_winner = true,
